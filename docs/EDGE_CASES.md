...
| Id | Case | Required behaviour |
|---|---|---|
| EC-42 | PII in transcript | Redaction on by default: digit runs of 4+, dates, emails, phone shapes, and everything captured during an entity turn. Raw retention requires `NOD_TRACE_RAW=1` plus an explicit per-session flag. |
