...
**Open, as of 17 Sep — three items, not one.** The plumbing is done: the driver, the
bootstrap, the chart and the labelling all work, and `make bench` runs here in about five
seconds over 120 clips.

1. **Track C audio does not exist.** Phase 0's fourth bullet was never done. The sweep is
   specified over Track A *and* Track C, and Track A does not substitute for it.
2. **The Track A corpus is not committed.** 36.3 MB across 120 wav files, against a 2.5 MB
   `.git` today.
3. **`make bench` does not run on a clean clone.** It needs the corpus, and the corpus
   needs `data/seed.wav`, which is gitignored and regenerated by macOS `say`. The CLI exits
   with that explanation rather than a traceback, but the exit criterion says "on a clean
   clone" and this does not meet it.
